...
function loadDigest() {
    ...
    let jqxhr = $.ajax({
        ...
        .done(function(data) {
            console.log(data);
            if (data.digest_type_id) {
                $("#digestSelect").val(data.digest_type_id);
                changeTemplate( $("#digestSelect").val() );
            } else {
                $('#modal-template').modal('show');
                $('#pick').click(function () {
                    let radioValue = $("input[name='templateRadios']:checked").val();
                    if (radioValue) {
                        $("#digestSelect").val(radioValue);
                        changeTemplate( $("#digestSelect").val() );
...
function bindEvents() {
    ...
    $(".new-template input, .new-template textarea, select").on('input',function () {
        contentChanged();
    });
...
function contentChanged() {
    ...
    $("#news .card").each(function () {
        ...
        if (img[0] != undefined && imgScr != undefined) {
            imageWidth = img[0].naturalWidth;
            imageHeight = img[0].naturalHeight;
            if (imageWidth > 0 && imageHeight >0){
                imageHeight =  Math.round( imageHeight / (imageWidth / 451) );
                imageWidth = 451;
            }
...
function makeEmail(news) {
    let articles = "";
    //let artAnalytics = artReformat = artDev = artInn  = [];
    let artAnalytics = news.filter( n => n.section == 'analytics');
    let artDev = news.filter( n => n.section == 'development');
    let artReformat = news.filter( n => n.section == 'reformat');
    let artRetail = news.filter( n => n.section == 'retail');
    let artInn = news.filter( n => n.section == 'innovation');
    let artBrand = news.filter( n => n.section == 'branding');
    let artBigArch = news.filter( n => n.section == 'bigarch');
    let artSmallArch = news.filter( n => n.section == 'smallarch');
    let artVisual = news.filter( n => n.section == 'visualcom');

    let newsGrouped = [artAnalytics, artDev, artReformat, artRetail,artInn, artBrand,artBigArch,artSmallArch,artVisual];
    newsGrouped = newsGrouped.filter( n => n.length > 0);
...
<td width="5%" valign="top"><img height="24" width="35" alt="" border="0" class="article-td" src="https://digest.vds.group/img/arrow-big2.png"></td>
<td valign="top" style="line-height: 25%;mso-table-lspace:0; mso-table-rspace:0;"><div style="mso-table-lspace:0; mso-table-rspace:0;font-size: 24px;font-weight: bold;padding: 0;margin:0;margin-bottom: 16px;font-weight:600;line-height:100%;color: black !important;">`+headerText+`</div></td></tr>`;

        el.forEach(function (el,index) {
            content = content + `<tr>
<td width="5%" valign="top"><img height="17" width="25" alt="" border="0"  class="article-td" src="https://digest.vds.group/img/bullet3.png"></td>
...
<td valign="top"><a class="article-daily" style="text-decoration: none; color:black;" href="#new`+index+`">`+ el.headline +`</a></td>
    ...
    let excerts = "";
    newsGrouped.forEach(function (newsGroup, index) {
        newsGroup.forEach(function (el, index) {
            excerts = excerts.concat('<a name="' + el.id + '" class="excert-header-daily" style="text-transform: uppercase;font-weight: 600;padding-bottom: 0;margin-bottom: 0;font-size: 16px;display:block;text-decoration: none; color:black;">' + el.headline + '</a>');
            excerts = excerts.concat(empty_space_15);
            if (el.img != undefined) {
                excerts = excerts.concat('<img border="0" width="451" height="' + el.height + '" alt="" border="0" style="display:block;margin:0 auto;max-width:595px;width:100%;height:auto;" src="' + el.img + '">');
                excerts = excerts.concat(empty_space_15);
            }

            excerts = excerts.concat('<div class="excert-daily" >' + el.excert + '</div>');
            if (el.href != "")
                excerts = excerts.concat('<p class="excert-link"><a href="' + el.href + '"><img height="30" width="109" alt="" border="0" src="' + btn_img + '" alt="Подробнее"></a></p>');
        });
    });

    let body = `<table  id="mail" style="max-width: 595px;width: 595px;" align="center" border="0" cellpadding="0" cellspacing="0">
<tr><td><div class=dvPreheader style="color:white !important;FONT-SIZE: 1px; OVERFLOW: hidden; MAX-WIDTH: 0px; DISPLAY: none !important; LINE-HEIGHT: 1px; MAX-HEIGHT: 0px; VISIBILITY: hidden; mso-hide: all; opacity: 0">`+ subHeaderText+`</div>
<img width="1" height="1" src="http://digest.vds.group/tracker" alt=""></td></tr>
                        <!-- Header -->
                        <tr class="header" align="center">
                            <td class="wbg" bgcolor="`+header_color+`">
                                <img height="64" width="206" class="header-img" src="` + header_img + `" alt="Дайджест VDS">
                            </td>
                        </tr>
                        <tr>
                            <!-- Content -->
...
console.log(email);
    ...
    let fullEmail = `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
+ email + '</body></html>';
    ...
    let list = $("input[name='list']:checked").val();
    let data = {
        'email' : fullEmail,
        'subject' : subject,
        'body' : body,
        'list' : list,
        'digest_type_id' : $("#digestSelect").val(),
    };
